...
runners.forEach(function (info) {
  fullName.push (`${info.first_name} ${info.last_name}`);
});
...
let allCaps = [];
runners.map(function(info) {
  allCaps.push(info.first_name.toUpperCase());
})
console.log(allCaps); 
...
let largeShirts = [];
runners.filter(function(info) {
  if (info.shirt_size === "L")
  return largeShirts.push(`${info.first_name} ${info.last_name} ordered a Large shirt`);
});
...
const tally = runners.reduce((total, info) => {return total += info.donation;}, 0);
...
// Problem 1 Need a party invite list for participants who donated over 200
let partyList = [];
runners.map(function(info) {
  if (info.donation > 200)
  return partyList.push(`${info.first_name} ${info.last_name} should be added to the guest list`);
  });
  console.log(partyList);

// Problem 2 Email list of people who ordered small shirts
let smallShirts = [];
runners.filter(function(info) {
  if (info.shirt_size === "S")
  return smallShirts.push(`${info.first_name} ${info.email}`);
});
console.log(smallShirts);

// Problem 3 number of people from Skinix company
let companySkinix = [];
runners.map(function(info) {
  if (info.company_name === "Skinix")
  return companySkinix.push(`${info.company_name}: ${info.first_name} ${info.last_name}`);
});
